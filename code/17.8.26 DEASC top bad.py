...
def mutation1(i, P, D, NP, F, L, U):
    # ดึงค่าฟิตเนสของประชากรทั้งหมดและเรียงลำดับจากดีไปแย่
    fitness_values = P[:, D]
    sorted_indices = np.argsort(fitness_values)
    
    # แบ่งครึ่งประชากร
    half_NP = NP // 2
    top_indices = sorted_indices[:half_NP]
    bad_indices = sorted_indices[half_NP:]
    
    # สุ่มเลือก x_top 2 ตัว และ x_bad 2 ตัว โดยไม่ให้ซ้ำกัน
    idx_top = np.random.choice(top_indices, 2, replace=False)
    idx_bad = np.random.choice(bad_indices, 2, replace=False)
    
    x_top1 = P[idx_top[0], :D]
    x_top2 = P[idx_top[1], :D]
    x_bad1 = P[idx_bad[0], :D]
    x_bad2 = P[idx_bad[1], :D]
    
    # สุ่ม xr1 จากประชากรทั้งหมด โดยไม่ให้ซ้ำกับตัวที่ i
    valid = False
    while not valid:
        r1 = np.random.choice(NP)
        if r1 != i:
            valid = True
    xr1 = P[r1, :D]
    
    # คำนวณสมการตามภาพ (image_e3726c.png)
    v = xr1 + F * (x_top1 - x_bad1) + F * (x_bad2 - x_top2) 
    
    # ตรวจสอบให้อยู่ในขอบเขต (Boundary checking)
    for k in range(D):
...
def DEASC_algorithm(P, D, NP, L, U, fitness, xb, fb, nf, mc, VTR, MAX_NFE, pc1, nc1, nc2, archive, success_nfs, EE, mm):
    ...
    for i in range(NP):
        ...
        v = mutation1(i, P, D, NP, F, L, U) # แก้: ใส่ argument P, D, NP ให้เรียงตาม definition
...
# --- Parameters Setup ---                                
# fitness = fitness5  ;D = 50; NP = 40; L = -5.12* np.ones(D); U = 5.12* np.ones(D)     # Rastrigin Function               
