...
def crossoverSDEASC(i, P, D, v, pc1):
    ww = np.random.rand()
    if ww < pc1 :
        ms = 1
        CR = 0 + np.random.rand() * 0.1  
...
def selectionSDEASC(u, i, P, D, fb, xb, nf, VTR, fitness, archive, n_archive, ms, nc1, nc2, found_success):
    fitness_P = P[i,D]
    fitness_u = fitness(u)
    nf += 1
...
def initial_seed(P, r_s):
    ...
    for ind in sorted_P:
        found = False
        for seed in seeds:
            if euclidean_distance(ind[:-1], seed[:-1]) <= r_s:
                found = True
                break
        if not found:
            seeds.append(ind)
...
def seed_in_r(P, seeds, r_s):
    species_dict = defaultdict(list)
    for ind in P:
        assigned = False
        for seed in seeds:
            dist = euclidean_distance(ind[:-1], seed[:-1])
            if dist <= r_s:
                species_dict[tuple(seed)].append(ind)
                assigned = True
                break
    return species_dict
...
def SDEASC_algorithm(P, D, NP, L, U, fitness, xb, fb, nf, mc, VTR, MAX_NFE, pc1, nc1, nc2, archive, success_nfs, EE, mm, r_s, min_species_size, n_archive, found_success):
    seeds = initial_seed(P, r_s)
    species_dict = seed_in_r(P, seeds, r_s)
    new_P = []
    
    for seed_tuple, members in species_dict.items():
        species_pop = np.array(members)
        seed_fitness = seed_tuple[-1]
            
        if len(species_pop) < min_species_size:
            num_to_add = min_species_size - len(species_pop)
            for _ in range(num_to_add):
                new_ind = np.array(seed_tuple[:-1]) + np.random.uniform(-r_s, r_s, D)
                new_ind = np.clip(new_ind, L, U)
                new_ind_fitness = fitness(new_ind)
                new_ind_full = np.append(new_ind, new_ind_fitness)
                species_pop = np.vstack([species_pop, new_ind_full])
                nf += 1
            
        species_size = len(species_pop)
        
        for i_spec in range(species_size):
            if mc or nf >= MAX_NFE:
                break
            ...
            v = mutation2(i_spec % NP, NP, F1, xs1, L, U) 
                
            # Crossover
            u, ms = crossoverSDEASC(i_spec % NP, P, D, v, pc1)
            fitness_u = fitness(u) 
            nf += 1

            # Check if u is too close to seed (Re-random to maintain diversity)
            if abs(fitness_u - seed_fitness) < 1e-12: 
                u = L + (U - L) * np.random.rand(D) 
                fitness_u = fitness(u)
                nf += 1
            
            # Selection
            P, xb, fb, nf, mc, archive, nc1, nc2, success_nfs = selectionSDEASC(u, i_spec % NP, P, D, fb, xb, nf, VTR, fitness, archive, n_archive, ms, nc1, nc2, found_success)
        ...
        new_P.extend(species_pop.tolist())

    #up pop
    if new_P:
        new_P_array = np.array(new_P)
        sorted_indices = np.argsort(new_P_array[:, -1])
        new_P_sorted = new_P_array[sorted_indices]
        P = new_P_sorted[:NP]
    
    return P, xb, fb, nf, mc, pc1, nc1, nc2, archive, success_nfs, mm, 0
...
# fitness = fitness33  ;D = 50; NP = 40; L = -10* np.ones(D); U = 10* np.ones(D)       # Levy Function 
# fitness = fitness34  ;D = 30; NP = 40; L = -10* np.ones(D); U = 10* np.ones(D)       # Levy Function  N.13
# fitness = fitness45  ;D = 10; NP = 40; L = -5* np.ones(D); U = 5* np.ones(D)       # Styblinski-Tang Function
...
for w in range(Max_runs):
    ...
    for g in range(Maxgen): 
        ...
        if randA <= 0.9 :
            ...
            P, xb, fb, nf, mc, pc1, nc1, nc2, archive, success_nfs, mm, i = SDEASC_algorithm(P, D, NP, L, U, fitness, xb, fb, nf, mc, VTR, MAX_NFE, pc1, nc1, nc2, archive, success_nfs, EE, mm, r_s, min_species_size, n_archive, found_success)
