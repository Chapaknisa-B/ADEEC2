...
def fitness21(x):    #8 Himmelblau's function 
    term1 = x[0]**2 + x[1] - 11
    term2 = x[0] + x[1]**2 - 7
    return (term1**2 + term2**2)

def fitness22(x):    #8 Himmelblau's function MD1
    term1 = x[0]**2 + x[1] - 11
    term2 = x[0] + x[1]**2 - 7
...
def fitness26(x):   # MD4
    term1 = x[0]**4 / 4 - 10 / 3 * x[0]**3  + 25 / 2 * x[0] **2 - 30 * x[0]
    return term1
...
def selectionDEASC(u, i, P, D, fb, xb, nf, fitness, archive, n_archive, ms, nc1, nc2, success_nfs):
    fitness_P = P[i,D]
    fitness_u = fitness(u)
    nf += 1
...
def DEASC_algorithm(P, D, NP, L, U, fitness, xb, fb, nf, mc, MAX_NFE, pc1, nc1, nc2, archive, success_nfs, EE, mm):
    ...
    for i in range(NP):
        ...
        P, xb, fb, nf, mc_new, archive, nc1, nc2, success_nfs = selectionDEASC(u, i, P, D, fb, xb, nf, fitness, archive, n_archive, ms, nc1, nc2, success_nfs)
        if mc_new:
            mc = True
...
# --- Parameters Setup ---                                
# fitness = fitness5  ;D = 10; NP = 40; L = -5.12* np.ones(D); U = 5.12* np.ones(D)     # Rastrigin Function               
...
fitness = fitness20 ;D = 2; NP = 40; L = np.array([-3,-2]); U = np.array([3,2])        # Six-Hump Camel  
fitness = fitness21 ;D = 2; NP = 40; L = -6* np.ones(D); U = 6* np.ones(D)            # Himmelblau's function G
fitness = fitness22 ;D = 2; NP = 40; L = -6* np.ones(D); U = 6* np.ones(D)            # Himmelblau's function MD1
fitness = fitness23 ;D = 4; NP = 40; L = -6* np.ones(D); U = 6* np.ones(D)            # Himmelblau's function MD2
fitness = fitness24 ;D = 4; NP = 40; L = -6* np.ones(D); U = 6* np.ones(D)            # Himmelblau's function MD3
fitness = fitness25 ;D = 2; NP = 40; L = -10* np.ones(D); U = 10* np.ones(D)          # MD4
...
for w in range(Max_runs):
    ...
    for g in range(Maxgen): 
        ...
        P, xb, fb, nf, mc, pc1, nc1, nc2, archive, success_nfs, mm, i = DEASC_algorithm(P, D, NP, L, U, fitness, xb, fb, nf, mc, MAX_NFE, pc1, nc1, nc2, archive, success_nfs, EE, mm)
