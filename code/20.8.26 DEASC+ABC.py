...
def mutation3(i, P, NP, D, L, U): 
    ...
    F1 = -1 + np.random.rand() * 2 
    F2 = 0 + 0.5 * np.random.rand()
    # F2 = 0 + np.random.rand() * 1.5

    # v = xi + F1 * (xi - xr1) + F2 * (xb - xi)
    v = xi + F1 * (xi - xr1)
...
def mutation4(i, P, NP, D, L, U, xs1): 
    valid = False
    while not valid:
...
def selectionDEASC(u, i, P, D, fb, xb, nf, VTR, fitness, ms, nc1, nc2, success_nfs):
    fitness_P = P[i,D]
    fitness_u = fitness(u)
    nf += 1
    ...
    return P, xb, fb, nf, mc, nc1, nc2, success_nfs 
...
def DEASC_algorithm(P, D, NP, L, U, fitness, xb, fb, nf, mc, VTR, MAX_NFE, pc1, nc1, nc2, success_nfs, EE, mm):
    ...
    for i in range(NP):
        ...
        M = np.random.rand()
        if M <= 0.8 :
            v = mutation1(i, NP, L, U)
            # v = mutation2(i, P, NP, D, L, U)

        else: 
            S = select_elite(P, D, NP)
            xs1 = P[random.choice(S), 0:D]
            v = mutation4(i, P, NP, D, L, U ,xs1)
            # v = mutation2(i, P, NP, D, L, U)
            # v = mutation3(i, P, NP, D, L, U)

        u, ms = crossoverDEASC(i, P, D, v, pc1)
                    
        P, xb, fb, nf, mc_new, nc1, nc2, success_nfs = selectionDEASC(u, i, P, D, fb, xb, nf, VTR, fitness, ms, nc1, nc2, success_nfs)
        if mc_new:
            mc = True
    ...
    return P, xb, fb, nf, mc, pc1, nc1, nc2, success_nfs, mm, i
...
# fitness = fitness7  ;D = 50; NP = 40; L = -32* np.ones(D); U = 32* np.ones(D)         # Ackley Function                    
...
for w in range(Max_runs):
    ...
    for g in range(Maxgen): 
        ...
        P, xb, fb, nf, mc, pc1, nc1, nc2, success_nfs, mm, i = DEASC_algorithm(P, D, NP, L, U, fitness, xb, fb, nf, mc, VTR, MAX_NFE, pc1, nc1, nc2, success_nfs, EE, mm)
